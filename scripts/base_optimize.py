...
def get_model(trial):
    ...
    investment_id_x = layers.Dense(64, activation=activation)(investment_id_x)
    investment_id_x = layers.Dense(64, activation=activation)(investment_id_x)
    investment_id_x = layers.Dense(64, activation=activation)(investment_id_x)
    
    feature_x = layers.Dense(256, activation=activation)(features_inputs)
    feature_x = layers.Dense(256, activation=activation)(feature_x)
    feature_x = layers.Dense(256, activation=activation)(feature_x)
    
    x = layers.Concatenate(axis=1)([investment_id_x, feature_x])
    x = layers.Dense(512, activation=activation, kernel_regularizer="l2")(x)
    x = layers.Dense(128, activation=activation, kernel_regularizer="l2")(x)
    x = layers.Dense(32, activation=activation, kernel_regularizer="l2")(x)
    output = layers.Dense(1)(x)
    rmse = keras.metrics.RootMeanSquaredError(name="rmse")
    model = tf.keras.Model(inputs=[investment_id_inputs, features_inputs], outputs=[output])
    model.compile(optimizer=tf.optimizers.Adam(learning_rate=0.001), loss='mse', metrics=['mse'])
...
study_name = 'swish_v_relu'  # Unique identifier of the study.
study = optuna.create_study(study_name=study_name, direction="minimize", storage='sqlite:///studies.db', load_if_exists = True)
study.optimize(objective, n_trials=30,  timeout = 21600)
df = study.trials_dataframe()
df.to_csv("base_optimization.csv")
joblib.dump(study_name, 'study_base.dump')
